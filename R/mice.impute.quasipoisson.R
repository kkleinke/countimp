#' Multiple Imputation of Overdispersed Count Data based on a \code{quasipoisson} GLM
#' 
#' Imputes univariate missing data based  on a \code{quasipoisson} GLM following either the Bayesian regression or bootstrap regression (appendix \code{.boot}) MI approach.
#' 
...
#' }
#' \code{quasipoisson} imputation relies on the standard \code{glm.fit} function, using the \code{quasipoisson} family. 
...
#' @aliases mice.impute.quasipoisson.boot mice.impute.qpois.boot mice.impute.qpois
#' @references 
#' \itemize{
...
#' }
#' @importFrom stats coef glm.fit quasipoisson rnbinom rpois summary.glm
...
#' @describeIn  mice.impute.quasipoisson Bayesian regression variant
#' @export
...
function (y, ry, x, wy = NULL, EV=TRUE, ...) 
{
  if (is.null(wy)) 
    ...
    p <- exp((x[wy, , drop = FALSE] %*% beta.star))
	  im=rnbinom(n=length(p),size=(p/(fit.sum$dispersion-1)),mu=p) 
	  imputed.values<-im
	  if(EV){
...
#' @export
#' @describeIn  mice.impute.quasipoisson identical to \code{mice.impute.quasipoisson()}; included for backward compatibility
mice.impute.qpois <-
  function (y, ry, x, wy = NULL, EV=TRUE, ...) 
  {
    if (is.null(wy)) 
...
#' @export
#' @describeIn  mice.impute.quasipoisson Bootstrap regression variant
mice.impute.quasipoisson.boot <-
  function (y, ry, x, wy = NULL, EV=TRUE, ...) 
  {
    if (is.null(wy)) 
      wy <- !ry
    ...
    xast<-xobs[sel,]
    yast<-yobs[sel]
    fit <- glm.fit(xast, yast, family = quasipoisson(link = log))
    fit.sum <- summary.glm(fit)
    beta.star <- coef(fit)
    p <- exp((x[wy, , drop = FALSE] %*% beta.star))
...
#' @export
#' @describeIn  mice.impute.quasipoisson identical to \code{mice.impute.quasipoisson.boot()}; included for backward compatibility
